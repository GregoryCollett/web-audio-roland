...
import { midiToName, NUM_BASS_STEPS } from '../../engine/bass/bassTypes';

interface BassStepGridProps {
  selectedStep: number;
  onSelectStep: (step: number) => void;
}

export function BassStepGrid({ selectedStep, onSelectStep }: BassStepGridProps) {
  const pattern = useBassPattern();
  const groups: number[][] = [];

  for (let g = 0; g < NUM_BASS_STEPS / 4; g++) {
    groups.push([g * 4, g * 4 + 1, g * 4 + 2, g * 4 + 3]);
  }

  return (
    <div className="bass-step-grid">
      <div className="bass-step-grid__rows">
        {groups.map((group, gi) => (
          <div key={gi} className="bass-step-grid__group">
            {group.map((stepIndex) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (step.gate === 'rest') {
                      bassEngine.setGate(stepIndex, 'note');
                    }
                  }}
                  onContextMenu={(e) => {
                    e.preventDefault();
