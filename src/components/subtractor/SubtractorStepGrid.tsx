...
import { midiToName, NUM_SUBTRACTOR_STEPS } from '../../engine/subtractor/subtractorTypes';

interface SubtractorStepGridProps {
  selectedStep: number;
  onSelectStep: (step: number) => void;
}

export function SubtractorStepGrid({ selectedStep, onSelectStep }: SubtractorStepGridProps) {
  const pattern = useSubtractorPattern();
  const groups: number[][] = [];

  for (let g = 0; g < NUM_SUBTRACTOR_STEPS / 4; g++) {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (step.gate === 'rest') {
                      subtractorEngine.setGate(stepIndex, 'note');
                    }
                  }}
                  onContextMenu={(e) => {
                    e.preventDefault();
                ...
                >
                  ...
                  {step.gate !== 'rest' && (
                    <span
                      className="bass-step__velocity-bar"
                      style={{
                        height: `${Math.round((step.velocity / 127) * 100)}%`,
                      }}
                    />
                  )}
